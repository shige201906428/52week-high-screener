...
def check_52week_high(ticker_list, lookback_days=15):
    """過去3週間（15営業日）の間に52週新高値を更新した回数をカウントし、直近3週間のローソク足、業種、空売り残高データを取得する"""
    results = []
        ...
        try:
            ...
            if high_count > 0:
                ...
                try:
                    ...
                    # 米国株のみ空売り残残高データを取得可能 (日本株.T の場合は yfinance から取得できないためスキップ)
                    if not ticker_symbol.endswith(".T"):
                        raw_short_pct = ticker_info.get("shortPercentOfFloat")
                        if raw_short_pct is not None:
                            short_pct = f"{raw_short_pct * 100:.2f}%"
                        
                        raw_short_ratio = ticker_info.get("shortRatio")
                        if raw_short_ratio is not None:
                            short_ratio = f"{raw_short_ratio:.2f}"
                except Exception:
                    pass
...
def generate_html_report(df, output_path, title_suffix=""):
    """index.htmlとして概要・チャート・直近3週間ローソク足・空売り残高付きレポートを出力する"""
    ...
    for idx, row in df.iterrows():
        ...
        <tr>
            ...
            <td class="text-center vertical-middle">{short_pct}</td>
            <td class="text-center vertical-middle">{short_ratio}</td>
            <td class="text-center vertical-middle">
                <canvas class="spark-candle" width="160" height="35" data-candles='{row['Candles']}'></canvas>
            </td>
            <td class="vertical-middle" style="min-width: 150px;">
                <input type="text" class="form-control form-control-sm memo-input" data-ticker="{ticker}" placeholder="メモを入力...">
            </td>
        </tr>
        """
...
<head>
    ...
    <style>
        ...
        .memo-input {{ border: 1px solid #ced4da; border-radius: 4px; padding: 4px 8px; font-size: 0.85rem; width: 100%; }}
        .memo-input:focus {{ border-color: #2ec4b6; box-shadow: 0 0 0 0.2rem rgba(46, 196, 182, 0.25); }}
...
<body>
    <div class="container">
        <div class="header-section">
            ...
            <small>最終更新日時: <strong>{now_str}</strong> | Hit数: {len(df)} 銘柄</small>
        </div>
                ...
                <table id="screenerTable" class="table table-hover table-striped table-bordered" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">空売り比率 (Short % of Float)</th>
                            <th class="text-center">ショートレシオ (Short Ratio)</th>
    ...
    <script>
        ...
        // 💡【追加】メモ欄のローカルストレージ同期処理
        function syncMemoFields() {{
            document.querySelectorAll('.memo-input').forEach(input => {{
                const ticker = input.getAttribute('data-ticker');
                
                // 1. localStorage から既存の書き込みデータを復元
                const savedMemo = localStorage.getItem('memo_' + ticker);
                if (savedMemo) {{
                    input.value = savedMemo;
                }}

                // 2. 文字が入力されたらリアルタイム（inputイベント）で自動保存
                input.addEventListener('input', (e) => {{
                    const currentTicker = e.target.getAttribute('data-ticker');
                    const val = e.target.value;
                    if (val.trim() === '') {{
                        localStorage.removeItem('memo_' + currentTicker);
                    }} else {{
                        localStorage.setItem('memo_' + currentTicker, val);
                    }}
                }});
            }});
        }}
        ...
        $(document).ready(function() {{
            const table = $('#screenerTable').DataTable({{
                ...
                "pageLength": 50,
                "columnDefs": [
                    {{ "orderable": false, "targets": [6, 7] }} // トレンドとメモ欄は並び替え対象外にする
                ],
                "language": {{
                    "search": "絞り込み検索:",
            ...
            // 💡【重要】DataTablesがページ切り替えや検索絞り込みで再描画した時にも毎回実行
...
if __name__ == "__main__":
    ...
    if result_df is None or result_df.empty:
        result_df = pd.DataFrame(columns=["Ticker", "High_Count", "Current_Price", "52W_High_Price", "Candles", "Sector", "Short_Pct", "Short_Ratio"])
...
#                     # 米国株のみ空売り残高データを取得可能 (日本株.T の場合は yfinance から取得できないためスキップ)
#                     if not ticker_symbol.endswith(".T"):
#                         raw_short_pct = ticker_info.get("shortPercentOfFloat")
#                         if raw_short_pct is not None:
#                             short_pct = f"{raw_short_pct * 100:.2f}%"
...
#                     "Candles": json.dumps(candles_data),
#                     "Sector": sector,
#                     "Short_Pct": short_pct,    # 💡【追加】
#                     "Short_Ratio": short_ratio  # 💡【追加】
#                 })
#         except Exception:
#             continue
...
# #                     <thead>
# #                         <tr>
# #                             <th>ティッカー / 業種 (Ticker / Sector)</th>
# #                             <th class="text-center">新高値カウント (過去3週間)</th>
# #                             <th>直近終値 (Current Price)</th>
# #                             <th>52週最高値 (52W High Price)</th>
# #                             <th class="text-center">直近3週間トレンド (日足)</th>
# #                         </tr>
# #                     </thead>
# #                     <tbody>
# #                         {table_rows}
